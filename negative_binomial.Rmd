...
$$
S_\lambda = \text{CV}(\lambda) = \frac{\sigma(\lambda)}{\mu(\lambda)} = \sqrt{\frac{\alpha}{\beta^2}} \frac{\beta}{\alpha} = \frac{1}{\sqrt{\alpha}}.
$$

This is unexpected, since under the binomial model, increases in social differentiation increase explained variance/social differentiation.
...
lk.nbinom <- function(par, x, d, priors=list(a=c(1, 0.5), b=c(1, 0.5))) {
  ...
  r <- par[1]
  p <- par[2]/(par[2] + d)
  sum(dnbinom(x, r, p, log=TRUE))#  + dgamma(par[1], priors$a[1], priors$a[2], log=TRUE) + dexp(par[2], 1e-2, log=TRUE)# dgamma(par[2], priors$b[1], priors$b[2], log=TRUE)
}
...
run_simulations <- function (iterations, generate_data, plot_chains=FALSE) {
  ...
  for (i in 1:iterations) {
    ...
    results[nrow(results) + 1, ] <- c(mean_sampling, cor_true, cor_est, cor_est_lb, cor_est_ub, ev_true, ev_est, ev_est_lb, ev_est_ub, a, b, cv_true)
  }
...
# Generate data using an exponential.
...
```{r}
results.pois <- run_simulations(200, generate_data_poisson, plot_chains=FALSE)
...
ggsave("figures/correlation_comparison_beta.png", width=3, height=3)
```
...
```{r}
results.norm <- run_simulations(100, generate_data_binom, plot_chains=FALSE)
